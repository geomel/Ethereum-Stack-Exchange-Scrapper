...
function storeResults(pname, pid){
			
		if(checkForDoubles(pname)){
			...
			// add to storage
			sessionStorage.setItem(currentCount.toString(), JSON.stringify(projectData()));
			// update storedCount var by one
			sessionStorage.setItem('storedCount', (currentCount + 1).toString());
			}			
	}

function checkForDoubles(pname){
if(sessionStorage.length!=0){
	for (var i = sessionStorage.length-1; i >= 0; i--) {
					var itemKey = sessionStorage.key(i);
					if(itemKey != 'storedCount') {
						var project = JSON.parse(sessionStorage.getItem(itemKey));
...
function getLocalStorageData(){
				...
				for (var i = sessionStorage.length-1; i >= 0; i--) {
					...
					var projectId = project.projectId;
						var projectName = project.projectName;
						// ... etc
						projectHtml += '<li><a href="_/php/_startProjectSession.php?pid='+projectId+'">'+projectName+'</a></li>';
						
					}
				}
				// append to the DOM using jQuery
				$('#recent-projects').append(projectHtml);
	}	
