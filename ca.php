...
<script type="text/javascript">
 ...
 function php2Js(){
	<?php
			...
			foreach($_SESSION['commits'] as $key=>$value){
				echo "commits[".$key."]=".$value.";";
			}
			foreach($_SESSION['filesAdded'] as $key=>$value){
				echo "filesAdded[".$key."]=".$value.";";
			}
			foreach($_SESSION['filesDeleted'] as $key=>$value){
				echo "filesDeleted[".$key."]=".$value.";";
			}
			foreach($_SESSION['filesModified'] as $key=>$value){
				echo "filesModified[".$key."]=".$value.";";
			}
			foreach($_SESSION['linesAdded'] as $key=>$value){
				echo "linesAdded[".$key."]=".$value.";";
			}
			foreach($_SESSION['linesDeleted'] as $key=>$value){
				echo "linesDeleted[".$key."]=".$value.";";
			}
			foreach($_SESSION['testFilesAdded'] as $key=>$value){
 ...
 function drawLinePlot(flag, ydata){
			...
			var options = {
				...
				switch(flag) {
						case "1":
							var nodes_plot = $.plot($("#authorschart"), [tableData], options);
							break;
						case "2":
							var edges_plot = $.plot($("#commitschart"), [tableData], options);
							break;
						case "3":
							var diameter_plot = $.plot($("#filesAddedchart"), [tableData], options);
							break;	
						case "4":
							var cc_plot = $.plot($("#filesDeletedchart"), [tableData], options);
							break;
						case "5":
							var e2n_plot = $.plot($("#filesModifiedchart"), [tableData], options);
							break;
						case "6":
							var ebtwne_plot = $.plot($("#linesAddedchart"), [tableData], options);
							break;
						case "7":
							var ebtwnn_plot = $.plot($("#linesDeletedchart"), [tableData], options);
							break;	
						case "8":
							var e2e_plot = $.plot($("#testFilesAddedchart"), [tableData], options);
							break;		
						case "9":
							var del_plot = $.plot($("#testFilesModifiedchart"), [tableData], options);
							break;
				}
